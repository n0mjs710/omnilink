...
rings, no handoffs (D-08), which is the point: a queue would be latency,
memory, and cache traffic bought for nothing while the daemon is I/O
bound. The contract is nonetheless the seam that would survive that
changing.
